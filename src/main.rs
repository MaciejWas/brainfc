...
fn main() {
    ...
    report_info(format!(
        "Done ({})",
        output_file.as_path().as_os_str().to_str().unwrap()
    ));
}
...
mod test {
    ...
    macro_rules! build_ir_test {
        ($name:ident) => {
            ...
            pub fn $name() {
                let input =
                    std::fs::read_to_string(format!("tests/ir_tests/{}.input", stringify!($name)))
                        .unwrap();
                let expected_output =
                    std::fs::read_to_string(format!("tests/ir_tests/{}.output", stringify!($name)))
                        .unwrap();
